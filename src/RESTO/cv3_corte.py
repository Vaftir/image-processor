...
def remove_background(image_cropper):
    if image_cropper.start_x is not None and image_cropper.start_y is not None:
        try:
            # Crop the selected area
            cropped_image = image_cropper.original_img.crop(
                (image_cropper.start_x, image_cropper.start_y, image_cropper.end_x, image_cropper.end_y)
            )
...
def main():
    ...
    button_process = ttk.Button(frame, text="Remover Fundo", command=lambda: remove_background(image_cropper), style="TButton")
    button_process.grid(row=6, column=0, columnspan=2, pady=10)

    image_cropper = ImageCropper(window, canvas_original_image, label_output_image)
